...
// Find the end of window.showMilestoneSchema
const searchString = "    if (window.lucide) window.lucide.createIcons();\n};\n";
const idx = code.indexOf("    document.body.appendChild(overlay);\n    if (window.lucide) window.lucide.createIcons();\n};");

if (idx !== -1) {
    // Cut off the file right at the end of the schema function
    const fixedCode = code.substring(0, idx + 98); // 98 is the length of the matched string
...
`;
    ...
    console.log("Fixed components.js successfully by hard splitting!");
} else {
