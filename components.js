...
window.renderAiAgent = (app, container) => {
    ...
    window.compileAiSystemPrompt = (app) => {
        try {
            const extractOfferId = (offerStr) => {
                if (!offerStr) return '';
            ...
            const mailers = app.state.mailers || [];
            const spamhaus = app.state.spamhaus || {};
            const vmtaResults = app.state.vmtaResults || {};
            const rpInventory = app.state.rpInventory || [];
            const ipDeliveryStatuses = app.state.statuses || {};
            const warmupData = app.state.warmupData || {};
            ...
            rpInventory.forEach(item => {
                if (item && item.rpDomain) {
                    rpDomainSet.add(item.rpDomain.trim().toLowerCase());
                }
            });
            ...
            const sendingDomainRevStats = {};
            const sendingTldRevStats = {};
            ...
            Object.entries(ipDeliveryStatuses).forEach(([safeIp, dates]) => {
                if (dates && dates[todayStr]) {
                    const status = dates[todayStr];
...
IP DELIVERY STATUS BREAKDOWN FOR TODAY (${todayStr}) (PRE-COMPUTED):
------------------
...
12. If the user asks about revenue by domain name extension, TLD, domain extension performance, or which extensions are most/least profitable, look at the REVENUE BY SENDING DOMAIN EXTENSION / TLD section above. If they ask about revenue per specific sending domain, look at the REVENUE BY SENDING DOMAIN section. Each domain is tagged as [Domain (RP)] if it matched a known RP domain, or [RDNS] if the drop had no returnPath. Also use the summary stats for RP vs RDNS revenue comparison. Present ranked results with revenue, drops count, EPC, and CPM in a table format.`;
            ...
            console.error("Failed to compile AI system prompt on client:", e);
            return "";
        }
    };
...
window.downloadWarmup24hReport = () => {
    ...
    let csvContent = "\ufeffIP Address,Domain,User,Send Size,Succeed (Last 3 Drops >=95% of IN),Next Tier Command,Test After Command,PMTA Command\n";
    ...
    active24Groups.forEach(g => {
        ...
        csvContent += `"${ip}","${domain}","${user}",${sendSize},"${succeed}","${nextCommand}","${nextTestCommand}","${pmtaCommand}"\n`;
    });
